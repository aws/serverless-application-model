...
The AWS Serverless Application Specification is licensed under [The Apache License, Version 2.0](http://www.apache.org/licenses/LICENSE-2.0.html).

## Introduction

AWS Serverless Application Specification is a specification used to define serverless applications on AWS. 
...
The AWS Serverless Application Specification is based on [AWS CloudFormation](https://aws.amazon.com/cloudformation/). A serverless application is defined in a [CloudFormation template](http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/gettingstarted.templatebasics.html) and deployed as a [CloudFormation stack](http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/updating.stacks.walkthrough.html). An AWS Serverless Application Specification template is a CloudFormation template.

The AWS Serverless Application Specification defines a set of objects which can be included in a CloudFormation template to describe common components of serverless applications easily.
...
NOTE: To specify an S3 bucket as an event source for a Lambda function, both resources have to be declared in the same template. AWS Serverless Application Specification does not support specifying an existing bucket as an event source.
