...
Configure Auth on APIs. Define Lambda and Cognito `Authorizers` and specify a `DefaultAuthorizer`.
...
Auth:
  ...
  Authorizers:
    MyCognitoAuth:
      Type: COGNITO_USER_POOLS # OPTIONAL; Defaults to 'COGNITO_USER_POOLS' when `UserPoolArn` is specified
