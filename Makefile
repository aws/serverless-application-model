...
black-check:
	...
	mkdir -p .tmp
	python -m samtranslator.schema_source.schema --sam-schema .tmp/sam.schema.json --cfn-schema schema_source/cloudformation.schema.json --unified-schema .tmp/schema.json
...
update-schema-data:
	# Parse docs
	bin/parse_docs.py .tmp/aws-sam-developer-guide/doc_source > samtranslator/schema_source/docs.json
	bin/parse_docs.py --cfn .tmp/aws-cloudformation-user-guide/doc_source > samtranslator/schema_source/cloudformation-docs.json

	# Add CloudFormation docs to CloudFormation schema
	python bin/add_docs_cfn_schema.py --schema .tmp/cloudformation.schema.json --docs samtranslator/schema_source/cloudformation-docs.json > schema_source/cloudformation.schema.json

schema:
	python -m samtranslator.schema_source.schema --sam-schema schema_source/sam.schema.json --cfn-schema schema_source/cloudformation.schema.json --unified-schema samtranslator/schema/schema.json
