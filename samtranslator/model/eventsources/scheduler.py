...
class SchedulerEventSource(ResourceMacro):
    ...
    property_types = {
        ...
        "Name": PropertyType(False, one_of(IS_STR, IS_BOOL)),
    ...
    def _construct_scheduler_schedule_without_target(
        ...
        # When `Name` is `False` explicitly,
        # we set `AWS::Scheduler::Schedule`'s Name to None.
        # When `Name` is not specified, to keep backward compatibility,
        # we use name generated by SAM instead of passing `None`:
        # https://github.com/aws/serverless-application-model/issues/3109
        if self.Name:
            scheduler_schedule.Name = self.Name
        elif self.Name is False:
            scheduler_schedule.Name = None
        else:
            scheduler_schedule.Name = self.logical_id
