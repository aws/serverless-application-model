...
class MultipleResourceFoundException(Exception):
    ...
    @property
    def message(self):
        return "Multiple resources found with resource type {}. Please get the resource by using the logical id"\
...
class BucketAlreadyExistsException(Exception):
    ...
    @property
    def message(self):
        return "S3 bucket {} already exists. Please delete the bucket and run tests again" \
            .format(self._s3_bucket_name)
...
class LogicalIdDoesNotFollowPatternException(Exception):
    ...
    @property
    def message(self):
        return "{} LogicalID of {} does not follow pattern. It should end with 10 hex characters generated by the SHA".format(self._logical_id_to_clean, self._resource_type)