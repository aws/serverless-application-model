...
class TestTranslatorEndToEnd(TestCase):
    ...
    def test_transform_success(self, testcase, partition_with_region):
        ...
        self.assertDictEqual(deep_sorted(output_fragment), deep_sorted(expected))
    ...
    def test_transform_invalid_document(self, testcase):
        ...
        self.assertEquals(error_message, expected.get('errorMessage'))

    def test_transform_unhandled_failure_empty_managed_policy_map(self):
        document = {
        ...
        self.assertEquals(error_message, 'Managed policy map is empty, but should not be.')

    def assert_metric_call(self, mock, transform, transform_failure=0, invalid_document=0):
        metric_dimensions = [
            {
                'Name': 'Transform',
                'Value': transform
    ...
    def test_swagger_body_sha_gets_recomputed(self):
        document = {
            'Transform': 'AWS::Serverless-2016-10-31',
            'Resources': {
                'Resource': {
        ...
        self.assertNotEqual(deployment_key, deployment_key_changed)

        # Now let's re-deploy the document without any changes. Deployment Key must NOT change
        output_fragment = transform(document, parameter_values, mock_policy_loader)
        self.assertEquals(get_deployment_key(output_fragment), deployment_key_changed)

    def test_swagger_definitionuri_sha_gets_recomputed(self):
        document = {
            'Transform': 'AWS::Serverless-2016-10-31',
            'Resources': {
                'Resource': {
...
class TestFunctionVersionWithParameterReferences(TestCase):
    ...
    def test_logical_id_remains_same_without_parameter_change(self):
        ...
        self.assertEquals(first_version_id, second_version_id)
