...
class TestServerlessAppPlugin_on_before_transform_template_translate(TestCase):
    ...
    @patch("samtranslator.plugins.application.serverless_app_plugin.SamTemplate")
    @patch("boto3.Session")
    def test_sar_throttling_doesnt_stop_processing(self, BotoSessionMock, SamTemplateMock):
        session_mock = Mock()
        session_mock.region_name = "us-east-1"
        session_mock.get_available_regions.return_value = ["us-east-1"]
    ...
    @patch("samtranslator.plugins.application.serverless_app_plugin.SamTemplate")
    @patch("boto3.Session")
    def test_unexpected_sar_error_stops_processing(self, BotoSessionMock, SamTemplateMock):
        session_mock = Mock()
        session_mock.region_name = "us-east-1"
        session_mock.get_available_regions.return_value = ["us-east-1"]
        BotoSessionMock.return_value = session_mock
    ...
    @patch("samtranslator.plugins.application.serverless_app_plugin.SamTemplate")
    @patch("boto3.Session")
    def test_sar_success_one_app(self, BotoSessionMock, SamTemplateMock):
        session_mock = Mock()
        session_mock.region_name = "us-east-1"
        session_mock.get_available_regions.return_value = ["us-east-1"]
        BotoSessionMock.return_value = session_mock
    ...
    @patch("samtranslator.plugins.application.serverless_app_plugin.SamTemplate")
    @patch("boto3.Session")
    def test_sleep_between_sar_checks(self, BotoSessionMock, SamTemplateMock):
        session_mock = Mock()
        session_mock.region_name = "us-east-1"
        session_mock.get_available_regions.return_value = ["us-east-1"]
        BotoSessionMock.return_value = session_mock
        ...
        self.assertEqual(self.plugin._get_sleep_time_sec.call_count, 1)
...
class TestServerlessAppPlugin_on_after_transform_template(TestCase):
    ...
    def test_sleep_between_sar_checks(self):
        ...
        plugin.on_after_transform_template("template")
        self.assertEqual(client.get_cloud_formation_template.call_count, 2)
        self.assertEqual(plugin._get_sleep_time_sec.call_count, 1)

class TestServerlessAppPlugin_on_before_and_on_after_transform_template(TestCase):
    @patch("samtranslator.plugins.application.serverless_app_plugin.SamTemplate")
    @patch("boto3.Session")
    def test_time_limit_exceeds_between_combined_sar_calls(self, BotoSessionMock, SamTemplateMock):
        session_mock = Mock()
        session_mock.region_name = "us-east-1"
        session_mock.get_available_regions.return_value = ["us-east-1"]
        BotoSessionMock.return_value = session_mock
