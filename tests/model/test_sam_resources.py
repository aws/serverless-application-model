...
class TestInvalidSamConnectors(TestCase):
    ...
    def test_unsupported_permissions_connector(self):
        ...
        with self.assertRaisesRegex(
            InvalidResourceException, ".+Unsupported 'Permissions' provided for connector from AWS::Lambda::Function to AWS::DynamoDB::Table; valid values are: Read, Write."
        ):
            connector.to_cloudformation(**self.kwargs)[0]
    ...
    def test_unsupported_permissions_connector_with_one_supported_permission(self):
        ...
        connector.Permissions = ["INVOKE"]
        with self.assertRaisesRegex(
            InvalidResourceException, ".+Unsupported 'Permissions' provided for connector from AWS::DynamoDB::Table to AWS::Lambda::Function; valid values are: Read."
        ):
            connector.to_cloudformation(**self.kwargs)[0]
    ...
    def test_unsupported_permissions_combination(self):
        ...
        connector.Permissions = ["Read"]
        with self.assertRaisesRegex(
            InvalidResourceException, "Unsupported 'Permissions' provided for connector from AWS::SQS::Queue to AWS::Lambda::Function; valid combinations are: Read \\+ Write."
        ):
            connector.to_cloudformation(**self.kwargs)[0]
