...
class TestApiGatewayV2Authorizer(TestCase):
    ...
    def test_create_authorizer_fails_with_disable_function_default_permissions_non_lambda(self):
        with pytest.raises(InvalidResourceException) as e:
            ApiGatewayV2Authorizer(
                api_logical_id="logicalId",
                name="authName",
                jwt_configuration={"config": "value"},
                authorization_scopes=["scope1", "scope2"],
                disable_function_default_permissions=True,
            )
        self.assertEqual(
            e.value.message,
            "Resource with id [logicalId] is invalid. "
            + "DisableFunctionDefaultPermissions must be defined only for Lambda Authorizer.",
        )
