...
# --- Parametrized: should RAISE (mutually exclusive violation) ---


@pytest.mark.parametrize(
    "rule_props, props, description",
    [
        # Top-level: bool =True
        (
            ["TopBool=True", "TopStr"],
...
def test_raises_when_condition_met(rule_props, props, description):
...
# --- Parametrized: should NOT raise (valid combinations) ---
...
def test_valid_when_condition_not_met(rule_props, props, description):
...
def test_error_message_includes_value_syntax():
    ...
    with pytest.raises(InvalidResourceException) as exc:
        resource.validate_before_transform(SchemaFixture)

    assert "'Nested.Flag=True'" in exc.value.message
    assert "'Nested.Tags'" in exc.value.message
    assert "together" in exc.value.message
