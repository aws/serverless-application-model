...
class MetricsIntegrationTest(TestCase):
    ...
    def test_publish_single_metric(self):
        ...
        metrics.publish()
        total_count = self.get_metric_data(namespace, 'TestCountMetric', dimensions,
                                           datetime(now.year, now.month, now.day),
                                           datetime(tomorrow.year, tomorrow.month, tomorrow.day))
        latency_avg = self.get_metric_data(namespace, 'TestLatencyMetric', dimensions,
                                           datetime(now.year, now.month, now.day),
                                           datetime(tomorrow.year, tomorrow.month, tomorrow.day), stat='Average')
    ...
    def get_metric_data(self, namespace, metric_name, dimensions, start_time, end_time, stat='Sum'):
        ...
        while retries > 0:
            retries -= 1
            response = self.cw_client.get_metric_data(MetricDataQueries=[{
