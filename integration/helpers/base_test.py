...
#   2. The test resources are defined in a single template without Parameters or other CloudFormation macros (this ensures we can represent transform using a transform test)
#   3. An identical transform test exists for the integration test template (this ensures we keep transforming the working template the same way)
